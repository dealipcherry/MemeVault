...
7. **Добавить функциональность загрузки мемов пользователями**
   * создать команду /add_meme
   * реализовать загрузку фото / гиф / видео
   * добавить сохранение файла и тегов в БД
     
8. **Покрыть проект тестами (pytest + pytest-asyncio)**
   * Unit-тесты
...
| Задача                                         | Время     |
| ---------------------------------------------- | --------- |
| Разобраться с aiogram inline mode              | 4–5 часов |
| Минимальная настройка проекта                  | 2 часа    |
| Реализация HTTP-взаимодействия (через aiogram) | 3 часа    |
| Настройка PostgreSQL + SQLAlchemy async        | 6–8 часов |
| Создание моделей и репозиториев                | 4–5 часов |
| Реализация SearchService                       | 3 часа    |
| Inline‑обработчик                              | 3 часа    |
| Функциональность загрузки мемов                | 5–6 часов |
| Написание тестов                               | 6–8 часов |
| Docker-конфигурация и деплой                   | 4 часа    |
| Подготовка документации                        | 4 часа    |

**Общее время: ~44–52 часов**
...
## 2. Схемы
---
### 2.1 Блок-схема работы бота

```mermaid
flowchart TD
A([Пользователь вводит: @bot + запрос]) --> B{Получен inline_query?}
B -->|Да| C[Парсим текст запроса]
B -->|Нет| Z[Игнорируем]
C --> D{Есть подходящие мемы?}
D -->|Да| E[Формируем список результатов]
D -->|Нет| F[Мемы не найдены]
E --> G[answerInlineQuery]
F --> G
G --> H([Telegram показывает результаты])
```
---

### 2.2 Последовательность обработки запросов
```mermaid
sequenceDiagram
...
erDiagram
    USER {
        int id PK "Идентификатор пользователя"
        string telegram_id "ID пользователя в Telegram"
        string username "Имя пользователя"
        datetime created_at "Дата регистрации"
    }
    
    MEME {
        int id PK "Идентификатор мема"
        string title "Название мема"
        string description "Описание"
        string file_id "ID файла в Telegram"
        string file_type "Тип файла: photo/gif/video"
        string tags "Теги через запятую"
        datetime created_at "Дата добавления"
        int user_id FK "Владелец мема"
    }
